...
class SolcastApi:
    ...
    def get_forecast_n_hour(self, hourincrement) -> int:
        ...
        try:
            ...
            m = sum(z[self._pv_estimate] for z in g) / len(g)
    ...
    def get_power_production_n_mins(self, minuteincrement) -> float:
        ...
        try:
            ...
            return int(m[self._pv_estimate] * 1000)
        except Exception as ex:
            return 0.0
    ...
    def get_peak_w_day(self, dayincrement) -> int:
        ...
        try:
            ...
            m = max(z[self._pv_estimate] for z in g)
            return int(m * 1000)
        except Exception as ex:
            return 0
    ...
    def get_peak_w_time_day(self, dayincrement) -> dt:
        ...
        try:
            ...
            return max((z for z in g), key=lambda x: x[self._pv_estimate])["period_start"]
        except Exception as ex:
            return None
    ...
    def get_remaining_today(self) -> float:
        ...
        try:
            ...
            return sum(z[self._pv_estimate] for z in g) / 2
        except Exception as ex:
            return 0.0
    ...
    def get_total_kwh_forecast_day(self, dayincrement) -> float:
        ...
        for idx in range(1, len(self._data_forecasts)):
            ...
            ret += (prev[self._pv_estimate] + curr[self._pv_estimate]) / 2 * diff_hours
            needed_delta -= delta
    ...
    def makeenergydict(self) -> dict:
        ...
        try:
            ...
            for v in self._data_forecasts:
                ...
                if v[self._pv_estimate] == 0.0:
                    if lastv > 0.0:
                        wh_hours[d] = round(v[self._pv_estimate] * 500,0)
                        wh_hours[lastk] = 0.0
                    lastk = d
                    lastv = v[self._pv_estimate]
                else:
                    if lastv == 0.0:
                        #add the last one
            ...
            for s in self._data['siteinfo']:
                ...
                for x in self._data['siteinfo'][s]['forecasts']:   
                    ...
                    if zz.date() < lastday and zz.date() > yesterday:
                        h = f"{zz.hour}"
                        if zz.date() == today:
                            tally += x[self._pv_estimate] * 0.5 * self._damp[h]
